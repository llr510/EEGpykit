...
def MVPAnalysis(files, var1_events, var2_events, excluded_events=[], scoring="roc_auc", output_dir='',
...
def run_with_cli():
    ...
    MVPAnalysis(files=files, var1_events=args.var1.split(','), var2_events=args.var2.split(','),
                excluded_events=args.excluded_events.split(','), scoring=args.scoring, output_dir=args.output,
                indiv_plot=args.indiv_plot, epochs_list=[], extra_event_labels=extra, jobs=args.jobs)
...
if '__main__' in __name__:
    ...
    files = files[:3]
    print(files)
    MVPAnalysis(files=files, var1_events=['Normal/Correct'], var2_events=['Obvious/Correct', 'Subtle/Correct'],
                excluded_events=[], scoring="roc_auc", output_dir='../analyses/MVPA/rads', indiv_plot=False,
