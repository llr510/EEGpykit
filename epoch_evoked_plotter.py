...
from mne.fixes import _is_last_row
...
from mne.viz.topomap import (_prepare_topomap_plot, plot_topomap, _get_pos_outlines,
...
def _plot_evoked(evoked, picks, exclude, unit, show, ylim, proj, xlim, hline,
    ...
    if plot_type == "image" and isinstance(group_by, dict):
        ...
        for sel in group_by:  # mne.. we loop over selections
            if sel not in axes:
...
def _plot_lines(data, info, picks, fig, axes, spatial_colors, unit, units,
    ...
    if selectable:
        for ax in np.array(axes)[selectables]:
            if len(ax.lines) == 1:
                continue
            text = ax.annotate('Loadingmne..', xy=(0.01, 0.1),
...
def plot_evoked(evoked, picks=None, exclude='bads', unit=True, show=True,
    ...
    mne. note:: If bad channels are not excluded they are shown in red.
    ...
    noise_cov : instance of Covariance | str | None
        ...
        mne. versionadded:: 0.16.0
    time_unit : str
        The units for the time axis, can be "ms" or "s" (default).

        mne. versionadded:: 0.16
...
def plot_evoked_white(evoked, noise_cov, show=True, rank=None, time_unit='s',
    ...
    References
    ----------
    mne. [1] Engemann D. and Gramfort A. (2015) Automated model selection in
           covariance estimation and spatial whitening of MEG and EEG
           signals, vol. 108, 328-342, NeuroImage.
    """
    from mne.cov import whiten_evoked, Covariance, _ensure_cov
    import matplotlib.pyplot as plt
    time_unit, times = _check_time_unit(time_unit, evoked.times)
...
def plot_snr_estimate(evoked, inv, show=True, axes=None, verbose=None):
    ...
    mne. versionadded:: 0.9.0
    """
    import matplotlib.pyplot as plt
    from mne.minimum_norm import estimate_snr
...
def plot_evoked_joint(evoked, times="peaks", title='', picks=None,
    ...
    mne. note:: Axes to plot in can be passed by the user through ``ts_args`` or
...
def _format_evokeds_colors(instances, cmap, colors):
    ...
    from mne.evoked import Evoked, _check_evokeds_ch_names_times
    ...
    for cond, items in instances.items():
        if isinstance(items, (Epochs, Evoked, BaseEpochs)):
    ...
    elif isinstance(list_of_values, BaseEpochs) or isinstance(list_of_values[0],
                                                              BaseEpochs):
        instances = _get_evokeds_from_epochs(instances, BaseEpochs)
...
def plot_compare_evokeds(evokeds, picks=None, colors=None,
    ...
    mne. cssclass:: table-bordered
    mne. rst-class:: midvalign
    ...
    for cond, evoked in evokeds.items():
        # _validate_type(cond, 'str', 'Conditions')
        _validate_type(cond, (str, list, tuple), 'Conditions')
    ...
    ch_types = list(set(_get_channel_types(info, picks=picks, unique=True))
    ...
    if not do_topo:
        ...
        from mne.viz.topo import iter_topography
